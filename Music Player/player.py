...
root.title('AUTOMATED THERAPY USING SMV')
...
# adding icon
root.iconphoto(False, PhotoImage(file='images/logo.png').subsample(2,2))
    ...
    try:
        if directory != '':
            ...
            if lb.size() == 0:
                ...
                messagebox.showinfo(title='Empty -(AUTOMATED THERAPY USING SMV)', message='This folder is empty!\nPlease select another folder.')  
                add_playlist()  
            prev_dir['val'] = directory       
        else:
            path_error = 'Choose a valid path!'
            messagebox.showerror(title='Path error -(AUTOMATED THERAPY USING SMV)', message=path_error)
...
img = PhotoImage(file='images/Bhoomi.png').subsample(2,2)

# creating frames
# left frame
left_frame = Frame(root, width=400, height=500, background='#F3F3F3', bd=0)
left_frame.grid(row=0, column=0)
left_frame.config(highlightthickness=0, borderwidth=0)

# right frame
right_frame = Frame(root, width=500, height=500, background='#F3F3F3', bd=0)
right_frame.grid(row=0, column=1)

Label(left_frame, image=img, bg='#F3F3F3').pack()

first_frame = Frame(right_frame, width=500, height=50, background='#F3F3F3')
first_frame.pack(anchor='nw', pady=8)

second_frame = Frame(right_frame, width=500, height=400, background='#F3F3F3')
second_frame.pack(anchor='nw')

third_frame = Frame(right_frame, width=500, height=400, background='#F3F3F3', pady=15)
...
lb_item = Label(third_frame, text='AUTOMATED THERAPY USING SMV', font='ubuntu 15', bg='#F3F3F3', fg='#545454')
lb_item.grid(row=0, column=0, columnspan=4, sticky='nw', pady=10)

# label for song status
status_time = Label(third_frame, text='00:00', bg='#F3F3F3', font='ubuntu 10', fg='#545454')
status_time.grid(row=1, column=0, sticky='nw') 

# label for song total length
total_time = Label(third_frame, text='00:00', bg='#F3F3F3', font='ubuntu 10', fg='#545454')
...
play_button = Button(third_frame, command=pause, image=play_image, bg='#F3F3F3', 
       borderwidth=0, activebackground='#F3F3F3', highlightthickness=0)
play_button.grid(row=3, column=1, sticky='news',pady=15)

# prev and next buttons
Button(third_frame, image=play_prev, font='ubuntu 20 bold', bg='#F3F3F3', command=previous,
       borderwidth=0, activebackground='#F3F3F3', highlightthickness=0).grid(row=3, column=0, sticky='ne', pady=15, padx=15) 
Button(third_frame, image=play_next, font='ubuntu 20 bold', bg='#F3F3F3', command=next,
       borderwidth=0, activebackground='#F3F3F3', highlightthickness=0).grid(row=3, column=2, sticky='nw', pady=15, padx=15)    
